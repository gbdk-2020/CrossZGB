...
uint8_t hicolor_start(const hicolor_data * p_hicolor, uint8_t hicolor_bank) NONBANKED {
	// check CGB
	#ifndef CGB
	p_hicolor; hicolor_bank;
	return 0;
	#else
	...
	if ((p_hicolor->height_in_tiles << 3) > DEVICE_SCREEN_PX_HEIGHT)
		scroll_limit = ((p_hicolor->height_in_tiles << 3) - DEVICE_SCREEN_PX_HEIGHT);
	else scroll_limit = 0;
	...
	set_interrupts((IE_REG | LCD_IFLAG) & ~TIM_IFLAG);
